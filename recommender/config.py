...
if BaseSettings:
    ...
    class Settings:  # pragma: no cover - simple fallback for unit tests
        ...
        kafka_security_protocol: str = field(
            default_factory=lambda: os.getenv("KAFKA_SECURITY_PROTOCOL", "SASL_SSL")
        )
        kafka_sasl_mechanism: str = field(
            default_factory=lambda: os.getenv("KAFKA_SASL_MECHANISM", "PLAIN")
        )
        kafka_api_key: Optional[str] = field(default_factory=lambda: os.getenv("KAFKA_API_KEY"))
        kafka_api_secret: Optional[str] = field(default_factory=lambda: os.getenv("KAFKA_API_SECRET"))
        reco_requests_topic: str = field(
            default_factory=lambda: os.getenv("REQUESTS_TOPIC", "aerosparks.requests")
        )
        ...
        schema_registry_url: Optional[str] = field(default_factory=lambda: os.getenv("SCHEMA_REGISTRY_URL"))
        schema_registry_key: Optional[str] = field(default_factory=lambda: os.getenv("SCHEMA_REGISTRY_KEY"))
        schema_registry_secret: Optional[str] = field(default_factory=lambda: os.getenv("SCHEMA_REGISTRY_SECRET"))
